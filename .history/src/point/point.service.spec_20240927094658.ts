  ...
  describe('Concurrency control', () => {
    it('userID lock test - charge and use', async () => {
        ...
        jest.spyOn(userTable, 'insertOrUpdate').mockImplementation(async (id, point) => {
            currentPoint = point; // Update currentPoint when insertOrUpdate is called
            return {
                id,
                point,
                updateMillis: Date.now(),
        ...
        expect(userTable.selectById).toHaveBeenCalledTimes(2); // Each call to charge and use will call selectById once
        expect(userTable.insertOrUpdate).toHaveBeenCalledTimes(2); // One for charge and one for use
